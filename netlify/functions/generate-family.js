/* ------------------------------------------------------------------ *
 *  Throughline — Family Engagement note (serverless "waiter")
 *
 *  Same pattern as generate.js: runs on Netlify, holds the Anthropic
 *  API key secretly (ANTHROPIC_API_KEY env var), forwards each request
 *  to the AI. The browser never sees the key.
 *
 *  This one turns a BCBA's rough family-engagement / caregiver-training
 *  notes into the three HiRasmus fields:
 *    1. Purpose of Meeting
 *    2. Meeting Summary
 *    3. BCBA Follow-Up
 *
 *  To improve it over time, add more example pairs to EXAMPLES below
 *  and redeploy. That is how it gets more on-style.
 * ------------------------------------------------------------------ */
...
// Few-shot training: built from real (de-identified) BCBA family-engagement
// notes. Inputs are faithful shorthand; outputs only clean & expand them.
// Add more pairs here to make the tool better and more on-style over time.
const EXAMPLES = [
  {
    // Concise note, single caregiver (Mom), biweekly, AAC focus.
    caregivers: ["Mom"],
  ...
  },
  {
    ...
    output: JSON.stringify({
      column1:
        "The purpose of the meeting was to discuss the learner's progress at home and in the center. The caregiver reported concerns about the learner attending school. The BCBA discussed interventions to implement to help the learner become school ready, as well as the learner's hesitation to engage in group activities.",
      column2:
        "The caregiver discussed the learner's progress at home and recent behaviors, including the learner's trauma associated with different people visiting. The BCBA discussed the learner's progress in the center. The caregiver and the BCBA discussed concerns about the learner attending school, and the BCBA discussed different strategies to help support the learner before attending school. The BCBA also discussed the learner's hesitation to join group activities and the impact this can have on the learner in a school setting.",
      column3:
        "The caregiver and the BCBA will meet in one week to discuss the learner's progress.",
    }),
  },
];

function caregiverInstruction(caregivers) {
  const list = Array.isArray(caregivers)
    ? caregivers.map((c) => String(c).trim()).filter(Boolean)
    : [];
  if (list.length === 0) {
    return "Caregiver(s) present: not specified. Refer to the caregiver generically as \"the caregiver\".";
  }
  if (list.length === 1) {
    return `Caregiver(s) present: ${list[0]} (primary). Refer to this caregiver using this relationship throughout the note.`;
  }
...
exports.handler = async (event) => {
  const headers = {
    "Access-Control-Allow-Origin": "https://aba-automation.netlify.app",
  ...
  if (!apiKey) {
    return {
      ...
      body: JSON.stringify({
        error:
          "The server is missing its API key. In Netlify, go to Site settings > Environment variables, add ANTHROPIC_API_KEY with your key, then redeploy.",
      }),
    };
  }
  ...
  try {
    ...
    if (Array.isArray(body.caregivers)) caregivers = body.caregivers;
  } catch (e) {
    return {
      statusCode: 400,
      headers,
  ...
  if (!input) {
    return {
      statusCode: 400,
      headers,
      body: JSON.stringify({ error: "No meeting notes were provided." }),
    };
  }
  ...
  if (input.length > 8000) {
    return {
      statusCode: 400,
      headers,
      body: JSON.stringify({ error: "Input exceeds the 8000-character limit. Please shorten your notes." }),
    };
  }
  ...
  for (const pattern of INJECTION_PATTERNS) {
    if (pattern.test(input)) {
      return {
        statusCode: 400,
        headers,
        body: JSON.stringify({ error: "Input contains disallowed content." }),
      };
    }
  }

  // Build few-shot turns. Each example's caregiver context is prepended to
  // its input so the model learns to honor the selected relationship labels.
  const fewshot = EXAMPLES.flatMap((ex) => [
    {
      role: "user",
      content: `${caregiverInstruction(ex.caregivers)}\n\nNOTES:\n${ex.input}`,
    },
    { role: "assistant", content: ex.output },
  ]);

  const userContent = `${caregiverInstruction(caregivers)}\n\nNOTES:\n${input}`;
  ...
  try {
    const aiRes = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({
        ...
        messages: [...fewshot, { role: "user", content: userContent }],
      }),
    });

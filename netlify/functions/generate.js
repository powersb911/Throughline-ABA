/* ------------------------------------------------------------------ *
 *  Throughline — serverless function (the "waiter")
 *
 *  This runs on Netlify's servers, NOT in the browser. It holds your
 *  Anthropic API key secretly (set as an environment variable named
 *  ANTHROPIC_API_KEY in your Netlify site settings) and forwards each
 *  request to the AI. Your key never reaches the browser, so no one
 *  visiting the site can see or steal it.
 *
 *  To improve the tool over time, add more great example notes to the
 *  EXAMPLES list below, then redeploy. That is how it "learns."
 * ------------------------------------------------------------------ */

// Which AI model to use.
//   claude-sonnet-4-6           -> best balance of quality + cost (recommended)
//   claude-haiku-4-5-20251001   -> cheaper + faster, slightly less polished
...
// Few-shot training: real (de-identified) notes covering distinct learner
// profiles. Inputs are faithful shorthand; outputs only clean & expand them.
...
discussed learner progress w tech. recommend maintaining same team + schedule — when team changes or extended time out, learner has brief increases in target behaviors.`,
    output: JSON.stringify({
      column1:
        "During the onset of supervision, the learner was completing table work tasks and did well with fill-in-the-blank missing letters. The learner was observed to engage in conversations and initiate peer play independently, and to wait for technicians prior to transitioning across environments. The learner does well with following instructions and accepting alternative outcomes, and works well when earning Sonic characters.",
      column2:
        "During the supervision session, the BCBA added a final target for gross motor imitation (spin then lunge). The learner has mastered the intraverbal target for describing steps in a sequence, which the BCBA removed from the daily session. The learner has maintained low rates of target behaviors and increasing skill-acquisition goals. The BCBA modeled data collection for response to problem size.",
      column3:
        "The BCBA and technician discussed the learner's progress. The BCBA recommends maintaining the same team and schedule, as the learner exhibits brief increases in target behaviors when the team changes or there are extended periods of time out.",
    }),
  },
  {
...
discussed not packing learner's AAC device when parent picks up — little to no desire to engage w device. want to shape pairing device w home/caregivers.`,
    output: JSON.stringify({
      column1:
        "During the onset of supervision, the learner was engaged in a tantrum when transitioning into the kitchen. The learner still requires support with manding for needs without prompts. The learner is observed to independently bring their AAC device to technicians when looking for a specific page.",
      column2:
        "The BCBA reviewed the target for receptive ID of reinforcing items. A positional prompt was added due to data variability. The BCBA provided support for learner tantrums when a non-preferred task demand was placed. The learner has also exhibited a fascination with a new button added to the AAC device for a reinforcing item.",
      column3:
        "The BCBA discussed not packing the learner's AAC device when the parent picks up, as the learner is showing little to no desire to engage with the device. The BCBA would like to shape pairing the device with home and caregivers.",
    }),
  },
];
...
exports.handler = async (event) => {
  ...
  if (!input) {
    return {
      statusCode: 400,
      headers,
      body: JSON.stringify({ error: "No session notes were provided." }),
    };
  }
  ...
  try {
    const aiRes = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({
        ...
        messages: [...fewshot, { role: "user", content: input }],
      }),
    });
